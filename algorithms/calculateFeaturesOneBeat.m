...
xexcl((max(1,round(npkPos-20*10^(-3)*samplerate)):round(npkPos+round(120*10^(-3)*samplerate)))',1)=0;
xexcl=find(xexcl);
...
xexcl=ones(length(xfit),1);
xexcl(max(1,(round(npkPos-round(120*10^(-3)*samplerate))):round(npkPos+20*10^(-3)*samplerate))',1)=0;
xexcl=find(xexcl);
fo = fitoptions('Method','NonlinearLeastSquares',...
    'Lower',[-1,-2.001,0,-2],...
    'Upper',[1,2,1,2],...
